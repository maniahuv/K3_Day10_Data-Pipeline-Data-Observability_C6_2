...
def build_test_set(df: pd.DataFrame, output_path, sample_size: int = 10, min_docs: int = 1) -> list[dict[str, Any]]:
    ...
    for _, row in selected_df.iterrows():
        ...
        summary_val = first_sentence(str(row.get("summary", "")))
        authors_val = str(row.get("authors_joined", ""))
        date_val = str(row.get("published", ""))
        categories_val = str(row.get("categories_joined", ""))
